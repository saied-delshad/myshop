...
# this is previous fa_calendar.py content
...
class Calverter:
    ...
    def gregorian_to_jd(self, year, month, day):
        ...
        tm = 0 if int(month) <= 2 else (-1 if int(self.leap_gregorian(year)) else -2)

        return (
            (int(GREGORIAN_EPOCH) - 1)
            + (365 * (int(year) - 1))
            + math.floor((int(year) - 1) / 4)
            + (-math.floor((int(year) - 1) / 100))
            + math.floor((int(year) - 1) / 400)
            + math.floor((((367 * int(month)) - 362) / 12) + int(tm) + int(day))
        )
    ...
    def islamic_to_jd(self, year, month, day):
        ...
        return (
            day + math.ceil(29.5 * (month - 1)) + (year - 1) * 354 + math.floor((3 + (11 * year)) / 30) + ISLAMIC_EPOCH
        ) - 1
    ...
    def jd_to_jalali(self, jd, sep="/"):
        ...
        return str(year) + sep + "{0:02}".format(month) + sep + "{0:02}".format(day)

    def jd_to_jalaliINTyearMonth(self, jd):
        "Calculate Jalali date from Julian day"
...
def gregorian_to_jalali2(date, sep="/"):
    ...
    jd = cal.gregorian_to_jd(int(year), int(month), 1)
